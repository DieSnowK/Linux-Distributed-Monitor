...
namespace monitor
{
    ...
    void MemMonitor::UpdateOnce(monitor::MonitorInfo* monitorInfo)
    {
        Utils::ReadFile mem_file("/proc/meminfo");
        MemInfo mem_info;
        ...
        while(mem_file.ReadLine(&mem_datas))
        {
            ...
            mem_datas.clear();
        } // end of while(mem_file.ReadLine(&mem_datas))

        auto mem_detail = monitorInfo->mutable_mem_info();

        mem_detail->set_used_percent((mem_info.total - mem_info.avail) * 1.0 / mem_info.total * 100.0);
        ...
        mem_detail->set_k_reclaimable(mem_info.kReclaimable / KBToGB);
        mem_detail->set_s_reclaimable(mem_info.sReclaimable / KBToGB);
        mem_detail->set_s_unreclaim(mem_info.sUnreclaim / KBToGB);
    }
